...
public class PathCreator : MonoBehaviour
{
    ...
    private void MoveChecker(List<CellComponent> pairs, int rowIndex, int columnIndex)
    {
        ...
        for (int i = 0; i < 2; i++, columnIndex = -columnIndex)
        {
            ...
            {
                DestroyChecker.Add(_cells[checkColumn, checkRow].Pair);
    ...
    // private void MoveBlackCheckers(List<CellComponent> pairs)
    // {
    //     if (_currentCells.Coordinate.Y + 1 < _cells.GetUpperBound(0) + 1 &&
    //         _currentCells.Coordinate.X + 1 < _cells.GetUpperBound(1) + 1)
    //     {
    //         if (_cells[_currentCells.Coordinate.X + 1, _currentCells.Coordinate.Y + 1].Pair == null)
    //         {
    //             pairs.Add(_cells[_currentCells.Coordinate.X + 1, _currentCells.Coordinate.Y + 1]);
    //         }
    //     }
    //
    //     if (_currentCells.Coordinate.Y + 1 < _cells.GetUpperBound(0) + 1 &&
    //         _currentCells.Coordinate.X - 1 >= 0)
    //     {
    //         if (_cells[_currentCells.Coordinate.X - 1, _currentCells.Coordinate.Y + 1].Pair == null)
    //         {
    //             pairs.Add(_cells[_currentCells.Coordinate.X - 1, _currentCells.Coordinate.Y + 1]);
    ...
    // }
    //
    // private void MoveWhiteCheckers(List<CellComponent> pairs)
    // {
    //     if (_currentCells.Coordinate.Y - 1 >= 0 &&
    //         _currentCells.Coordinate.X + 1 < _cells.GetUpperBound(1) + 1)
    //     {
    //         if (_cells[_currentCells.Coordinate.X + 1, _currentCells.Coordinate.Y - 1].Pair == null)
    //         {
